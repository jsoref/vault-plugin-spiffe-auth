...
## How an application obtains a SPIFFE SVID
This information is based on my current understanding and needs validation by the SPIFFE team.

1. A SPIRE server is running at a central location and configured with a X509 certificate and key, which is capable of signing leaf certs for applications.  These leaf certs contain the SPIFFE ID also known as a SVID or SPIFFE Verifiable Identity Document.
2. To request an SVID the local spire agent first joins the SPIRE server, using a one shot token.  The token is generated by running the command `spire-server token generate -spiffeID spiffe://example.org/host`.  This entitles the agent to request an SVID for the given SPIFFE Trust Domain.
3. Before the agent can request an SVID the Spire server must be configured with the SPIFFE ID, this contains a selector which is a know mapping on the agent to the ID, in this example we are using the GUID of the user.  (Needs clarification)
...
4. The agent can then request an SVID using the following command `su -c "spire-agent api fetch -write /out" workload`.  Because the previous step configures the ID using a user GUID, running the fetch command with that user and an agent which has successfully joined a trust domain would return a valid SVID.
5. The returned package from the command in step 4 contains a CA bundle, SVID (x.509 cert), and a private key.  This information must be kept secret otherwise as the holder can use this document to authenticate to the Vault server.

## Authentication Flow with Vault
